    ...
    getProjectFiles(process.cwd(), function() {
      ...
      for (var i = projectData.length-1; i > 0; i--) {
        for (var key in projectData[i]) {
          //for projects before the first one encountered, only copy specified data
          if (isKeyValid(key)) {
      ...
      //for the first project encountered, copy all of the above and a few more
      for (var key in projectData[i]) {
        if (isKeyValid(key) || key === "container" || key ==="exclude") {
          app.project[key] = projectData[i][key];
        }
      }
